...
export default {
  ...
  watch:{
    isShowMiniMusic(val){
      ...
      if(val && this.isPlaying){
        if(!this.timer) this.timer = setInterval(this.rotateImage,100)
      }else{
    ...
    isPlaying(val){
      if(val && this.isShowMiniMusic) {        
        if(!this.timer) 
          {
            this.timer = setInterval(this.rotateImage,100)
            //console.log('paly'+this.timer)
          }
      }else{
        if(this.timer){
  ...
  methods: {
    ...
    rotateImage(){
      ...
      while(this.degnum >= 360){
        this.degnum = this.degnum - 360
      }
      this.degnum = this.degnum + 3
      //console.log('rotate('+this.degnum+'deg)')
      this.$refs.img.style.transform = 'rotate('+this.degnum+'deg)'
    },
    showPlay() {
